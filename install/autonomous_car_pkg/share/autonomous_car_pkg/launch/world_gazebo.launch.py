...
def generate_launch_description():
    ...
    pkg_share_dir = get_package_share_directory('autonomous_car_pkg')
    model_pkg_share_dir = get_package_share_directory('autonomous_car_pkg_models')
    ...
    spawn_red_light = ExecuteProcess(
        cmd=['ros2', 'run', 'autonomous_car_pkg', 'spawner_node', red_light_sdf,'red_light'],
        output='screen'
    )

    spawn_yellow_light = ExecuteProcess(
        cmd=['ros2', 'run', 'autonomous_car_pkg', 'spawner_node', yellow_light_sdf,'yellow_light'],
        output='screen'
    )

    spawn_green_light = ExecuteProcess(
        cmd=['ros2', 'run', 'autonomous_car_pkg', 'spawner_node', green_light_sdf,'green_Light'],
        output='screen'
    )
